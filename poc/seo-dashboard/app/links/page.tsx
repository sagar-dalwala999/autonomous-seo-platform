...
import { buildLinkRows } from "@/lib/data-links";
...
export default async function LinksPage({ searchParams }: Props) {
  ...
  const [rows, pages] = await Promise.all([buildLinkRows(runId), getPages(runId)]);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-secondary">
        Run <span className="font-medium text-foreground">{runId}</span> · {rows.length} unique link destination{rows.length === 1 ? "" : "s"}
      </p>
      {rows.length === 0 ? (
